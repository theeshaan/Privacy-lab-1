...
static pfil_return_t icmp_block_hook(pfil_packet_t packet, struct ifnet *ift, int dr, void *context, struct inpcb *inpb)
{
    struct mbuf *m;
    struct ip *ip_header;
    ...
    if (ip_header->ip_p == IPPROTO_ICMP) {
        ...
        if (icmp_header->icmp_type == ICMP_ECHO) {
            ...
            printf("[icmp_block] Dropped ICMP Echo Request #%lu (Size: %d bytes)\n", 
